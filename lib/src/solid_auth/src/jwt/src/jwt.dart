...
class JWT {
  ...
  static JWT verify(
    ...
    try {
      ...
      if (checkHeaderType && header['typ'] != 'dpop+jwt') {
        throw JWTInvalidError('not a jwt');
      }

      final algorithm = JWTAlgorithm.fromName(header['alg'].toString());
      ...
      if (payload is Map) {
          ...
          if (payload.containsKey('aud')) {
            if (payload['aud'] is String && payload['aud'] != audience.first) {
              throw JWTInvalidError('invalid audience');
                ...
                !const ListEquality().equals(payload['aud'] as List<dynamic>?, audience)) {
              throw JWTInvalidError('invalid audience');
            }
          }
          else{
        ...
        return JWT(
          ...
          issuer: payload.remove('iss') as String,
          subject: payload.remove('sub') as String,
          jwtId: payload.remove('jti') as String,
        );
      } else {
        return JWT(payload);
      }
  ...
  String sign(
    ...
    try {
      ...
      if (payload is Map<String, dynamic> || payload is Map<dynamic, dynamic>) {
        try {
          payload = Map<String, dynamic>.from(payload as Map<dynamic, dynamic>);
      ...
      try {
        b64Payload = base64Unpadded(
          payload is String
              ? base64.encode(utf8.encode(payload as String))
